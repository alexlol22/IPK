...
class SDLCanvas {
 ...
 public:
  ...
  SDLCanvas(const std::string& name, int winWidth, int winHeight) {
    ...
    if (numCanvas == 0) {
      if (SDL_WasInit(SDL_INIT_VIDEO) > 0)
        throw std::runtime_error(
            "SDL already initialized, only one instance allowed");
    ...
    if (not window) throw std::runtime_error("Failed to create SDL window");

    // renderer == interface between canvas and display
    renderer = SDL_CreateRenderer(window, -1, 0);
    if (not renderer) throw std::runtime_error("Failed to create SDL renderer");

    // texture == canvas
    texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
                                SDL_TEXTUREACCESS_TARGET, winWidth, winHeight);
    if (not texture) throw std::runtime_error("Failed to create SDL texture");
  ...
  // don't copy objects, use references instead
  SDLCanvas(const SDLCanvas& other) = delete;
  ...
  void drawPixel(int x, int y) { SDL_RenderDrawPoint(renderer, x, y); }
  ...
  bool windowClosed() {
    ...
    if (keystates[SDL_SCANCODE_ESCAPE]) return true;
  ...
  void drawCircle(int32_t centreX, int32_t centreY, int32_t radius, std::array<int, 3> color)
  {
    SDL_SetRenderDrawColor(renderer, color[0], color[1], color[2], 0x00);
