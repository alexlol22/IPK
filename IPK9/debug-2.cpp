...
class Circle : public Shape {
 public:
  ...
  //better use override specifier to prevent this from happening
  virtual int dimension() const { return 2; }
...
int main(int argc, char** argv) {
  ...
  // add some shapes
  shapes.push_back(std::make_unique<Point>(2, 4));
  shapes.push_back(std::make_unique<Circle>(70));
  shapes.push_back(std::make_unique<Sphere>(10));
  std::vector<Point> corners = {{-4., -4.}, {4., -4.}, {4., 4.}, {-4., 4.}};
  shapes.push_back(std::make_unique<Polygon>(corners));
  corners.resize(1);
  shapes.push_back(std::make_unique<Polygon>(corners));
