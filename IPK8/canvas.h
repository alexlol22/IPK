...
class Canvas {
 ...
 public:
  Canvas(const Point& center, double width, double height, int horPixels,
         int vertPixels)
      : _center(center),
        _width(width),
        _height(height),
  ...
  int horPixels() const { return _horPixels; }

  int vertPixels() const { return _vertPixels; }
  ...
  int operator()(int i, int j) const { return _pixels[i][j]; }

  // read/write access to pixel
  // to use it with a variable called mycanvas,
  // write mycanvas(i_coord,j_coord) = value;
  int& operator()(int i, int j) { return _pixels[i][j]; }
